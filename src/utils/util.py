...
def draw_horizon(image_grayscaled):
    horizon_x1, horizon_x2, horizon_y1, horizon_y2 = detect_horizon_line(image_grayscaled)
    color = (0, 0, 255)
    annotated = cv2.line(image_grayscaled, (horizon_x1, horizon_y1), (horizon_x2, horizon_y2), color, 2)
    return annotated
...
def detect_horizon_line(image_grayscaled):
    ...
    horizon_y1 = max(numpy.where(image_closed[:, horizon_x1] == 0)[0])
    horizon_y2 = max(numpy.where(image_closed[:, horizon_x2] == 0)[0])
